final Map<String, String> arSa = {
  'lbl_welcome_to_fruitables': 'مرحبًا بك في فريتابلز \u{1F44B}',
  'lbl_please_select_order_type': 'يرجى اختيار نوع الطلب للمتابعة',
  'lbl_delivery': 'توصيل',
  'lbl_pickup': 'استلام',
  'lbl_please_select_location': 'يرجى اختيار موقعك',
  'lbl_city_region': 'المدينة / المنطقة',
  'lbl_select_your_city': 'اختر مدينتك',
  'lbl_area_sub_region': 'الفروع',
  'lbl_select_your_area': 'اختر منطقتك',
  'lbl_confirm_location': 'تأكيد الموقع',
  "enter_username": "أدخل الاسم الكامل",
  "enter_phone_number": "هاتف #",
  'lbl_done': 'تم',
  'lbl_search': 'بحث',
  'lbl_close': 'إغلاق',
  'lbl_no_results': 'لم يتم العثور على نتائج',
  'lbl_no_cities_available': 'لا توجد مدن متاحة حالياً، يرجى المحاولة لاحقاً',
  'lbl_small': 'صغير',
  'lbl_medium': 'متوسط',
  'lbl_large': 'كبير',
  'lbl_new': 'جديد',
  'lbl_hot': 'حار \u{1F525}',
  'lbl_trending': 'أعلى تتجه',
  'lbl_from': 'ابدأ من',
  'lbl_rs': 'SAR',
  'lbl_off': 'خصم',
  'lbl_view_all': 'عرض الكل',
  'lbl_deliver_to': 'التوصيل إلى',
  'lbl_my_wallet': 'محفظتي',
  'lbl_loyalty_points': 'نقاط الولاء',
  'lbl_my_addresses': 'عناويني',
  'lbl_my_orders': 'طلباتي',
  'lbl_my_favourites': 'مفضلتي',
  'lbl_reviews': 'التعليقات',
  'whats_your_rate': "ما هو معدلك",
  'write_your_review': "اكتب رأيك",
  'review_submitted': 'تم تقديم المراجعة',
  'vouchers': 'قسائم',
  'minimum': 'الحد الأدنى',
  'valid_until': 'صالحة حتى',
  'current': "حاضِر",
  'past': "ماضي",
  "no_voucher_Subheading":
      "لا تتوفر قسائم في الوقت الحالي، ولكن ترقبوا العروض المثيرة!",
  "no_voucher_found": "لم يتم العثور على قسيمة",
  'code_copied': 'تم نسخ رمز القسيمة إلى الحافظة',
  'rating_required': "التقييم مطلوب",
  'would_you_like_to_write': 'هل ترغب في كتابة أي شيء عنا',
  'remove': "يزيل",
  'lbl_support_center': 'مركز الدعم',
  'lbl_logout': 'تسجيل الخروج',
  "how_was_your_last_order?": "ما هو طلبك الأخير؟",
  'lbl_req_account_deletion': 'طلب حذف الحساب',
  'lbl_change_language': 'تغيير اللغة',
  'lbl_add_to_cart': 'أضف إلى السلة',
  'lbl_view_cart': 'عرض السلة',
  'lbl_cart': 'السلة',
  'lbl_change': 'تغيير',
  'lbl_use_wallet': 'استخدام المحفظة',
  'lbl_apply_promo': 'تطبيق العرض الترويجي',
  'lbl_subtotal': 'المجموع الفرعي',
  'lbl_total': 'الإجمالي',
  'lbl_grand_total': 'المجموع الكلي',
  'lbl_delivery_fee': 'رسوم التوصيل',
  'lbl_tax': 'الضريبة',
  'lbl_size': 'الحجم',
  'lbl_proceed_to_checkout': 'المتابعة إلى الدفع',
  'lbl_search_products': 'البحث عن المنتجات',
  'lbl_warning': 'تحذير',
  'lbl_are_you_sure_to_logout': 'هل أنت متأكد من تسجيل الخروج؟',
  'lbl_no': 'لا',
  'lbl_yes': 'نعم',
  'lbl_login': 'تسجيل الدخول',
  'lbl_hi_guest': 'مرحباً ضيف ريكسا كافيه',
  'lbl_are_you_sure': 'هل أنت متأكد؟',
  'lbl_once_you_confirm_delete_account':
      'عند تأكيدك، سيتم تسجيل خروجك ولن تتمكن من الوصول إلى حسابك، وسيتم حذف جميع بيانات حسابك نهائيًا.',
  'lbl_add_address': 'إضافة عنوان',
  'lbl_delete_address': 'حذف العنوان',
  'lbl_are_you_sure_to_delete_address': 'هل أنت متأكد من حذف هذا العنوان؟',
  'lbl_for_queries': 'للاستفسارات، يرجى الاتصال بنا على:',
  'lbl_checkout': 'الدفع',
  "cash_on_delivery": "الدفع عند الاستلام",
  "credit_debit_card": "بطاقة الائتمان/الخصم",
  "payment_options": "خيارات الدفع",
  "price_exclusive_tax": "السعر بدون ضريبة",
  "today": "اليوم",
  "cancel": "إلغاء",
  "customer_details": "تفاصيل العميل",
  "full_name": "الاسم الكامل",
  "phone_number": "رقم الهاتف",
  "email": "البريد الإلكتروني",
  "delivery_address": "عنوان التوصيل",
  "additional_notes": "ملاحظات إضافية",
  "add_instructions": "أضف أي تعليمات لطلبك",
  "payment_method": "طريقة الدفع",
  "choose_payment_method": "اختر طريقة الدفع",
  "choose": "اختر",
  "asap": "في أسرع وقت",
  "confirm_order": "تأكيد الطلب",
  "select_payment_method": "يرجى اختيار طريقة الدفع",
  "select_delivery_address": "يرجى اختيار عنوان التوصيل",
  "enter_email": "أدخل البريد الإلكتروني",
  "send_email": "إرسال البريد الإلكتروني",
  "enter_otp": "يرجى إدخال رمز التحقق المرسل إلى بريدك الإلكتروني",
  "verify": "تحقق",
  "otp_required": "رمز التحقق مطلوب",
  "enter_valid_otp": "يرجى إدخال رمز تحقق صالح",
  "hide_categories": "إخفاء الفئات",
  "view_all_categories": "عرض جميع الفئات",
  "confirmed": "تم التأكيد",
  "order_details": "تفاصيل الطلب",
  "order": "الطلب",
  "item_list": "قائمة العناصر",
  "order_placed": "تم تقديم الطلب",
  "delivered": "تم التوصيل",
  "in_progress": "قيد التنفيذ",
  "cancelled": "تم الإلغاء",
  "status": "الحالة",
  "estimated_delivery_time": "الوقت المقدر للتوصيل",
  "save_address": "حفظ العنوان",
  "new_address": "عنوان جديد",
  "address": "العنوان",
  "ok": "موافق",
  "confirm_pin_location": "تأكيد موقع النقطة",
  "looks_like": "يبدو أن الموقع المحدد خارج المنطقة المختارة",
  "to_change_area": "لتغيير منطقتك يرجى القيام بذلك من القائمة الرئيسية",
  "email_cannot_be_empty": "البريد الإلكتروني لا يمكن أن يكون فارغاً",
  "invalid_email": "البريد الإلكتروني غير صالح",
  "exceeding_character_limit": "تجاوز الحد الأقصى للأحرف، يرجى تجربة آخر",
  "field": "الحقل",
  "cannot_be_empty": "لا يمكن أن يكون فارغاً",
  "is_required": "مطلوب",
  "you_will_receive_6_digit_otp":
      "ستتلقى رمز تحقق مكون من 6 أرقام للتحقق من البريد الإلكتروني",
  "otp_verification": "التحقق من رمز التحقق",
  "we_sent_otp_to_email": "لقد أرسلنا OTP إلى هاتفك",
  "variation": "اختر الحجم",
  "select_option": "يرجى اختيار خيار واحد للمتابعة",
  "items": "العناصر",
  "msg_didn_t_get_code": "إعادة إرسال رمز التحقق في ",
  "resend": "إعادة الإرسال",
  "seconds": "ثانية",
  "msg_server_not_responding": "الخادم لا يستجيب",
  "which_outlet_would_you_pickup_from": "من أي فرع ترغب في الاستلام؟",
  "pickup_from": "استلام من",
  "show_on_map": "عرض على الخريطة",
  "estimated_pickup_time": "الوقت المقدر للاستلام",
  "locate_me": "تحديد موقعي",
  "branch": "الفرع",
  "lbl_no_favorite": "لا توجد مفضلة هنا بعد!",
  "lbl_no_fav_description":
      "قائمة المفضلة لديك في انتظارك، ابحث عما تحبه واحفظه هنا.",
  "no_order_found": "لم يتم العثور على الطلبات!",
  "no_order_found_desc": "يبدو أنه فارغ هنا، فلنملأه بشيء مذهل!",
  "no_address_found": "لم يتم العثور على عناوين!",
  "no_address_found_desc": "لم تقم بإضافة عنوان بعد. لنبدأ بإضافة واحدة الآن!",
  "view_details": "عرض التفاصيل",
  "your_order_being_prepared": "طلبك قيد الإعداد",
  "help": "مساعدة",
  "lbl_discount": "خصم",
  "lbl_bottle": "زجاجة",
  "order_number": "رقم الطلب #",
  "your_order_being": "طلبك قيد",
  "mins": "دقائق",
  "preparing_order": "إعداد طلبك",
  "your_order": "طلبك",
  "when_its_ready": "عندما يكون جاهزاً، سيأخذ السائق طلبك",
  "signup": "التسجيل",
  "enter_otp1": "رمز التحقق",
  "floor": "الطابق",
  "street": "الشارع",
  "label": "التسمية",
  "home": "المنزل",
  "work": "العمل",
  "partner": "الشريك",
  'reorder': "حدد العناصر لإعادة ترتيبها",
  "other": "أخرى",
  "note_to_rider": "ملاحظة إلى السائق",
  "order_created": "تم إنشاء الطلب بنجاح",
  "add_address_amd_select_label": "يرجى إضافة العنوان واختيار التسمية",
  "instructions": "التعليمات",
  "none": "لا شيء",
  "order_type": "نوع الطلب",
  "login_to_confirm_order": "قم بتسجيل الدخول لتأكيد الطلب",
  "login_to_apply_promo": "قم بتسجيل الدخول لتطبيق العرض الترويجي",
  "lbl_order_from": "الطلب من",
  "pending": "قيد الانتظار",
  "preparing": "التحضير",
  "ready": "مستعد",
  "created_at": "تم إنشاؤها في",
  "delivered_on": "تسليمها في",
  "you_rated": "لقد قيمت هذا",
  "from_wallet": "من المحفظة",
  "from_points": "من النقاط",
  "paid_with": "دفعت مع",
  "add_review": "اكتب مراجعة",
  "calories": "سعرات حرارية",
  "lbl_use_points": "استخدام النقاط",
  'voucherCode': "أضف رمز القسيمة",
  "search_your_desire": "ابحث عن رغبتك",
  "you_can_search_using": "يمكنك البحث باستخدام عنوان المنتج أو وصف المنتج",
  "no_product_matched": "لم يتم تصنيع أي منتج",
  "please_double_check": "يرجى التحقق مرة أخرى من هجاءك هل هو صحيح",
};
